...
import { ShieldCheck, Users, UserCheck, UserX, ToggleLeft, ToggleRight, Bell, Send } from 'lucide-react';
...
export default function AdminPage() {
    ...
    return (
        <DashboardLayout>
            <Header title="Admin Panel" subtitle="User management and system overview" />

            <div className="px-8 py-6 space-y-8">

                {/* Stats */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <StatCard label="Total Users" value={total} sub={`${admins} admin(s)`} icon={Users} color="blue" />
                    <StatCard label="Active Users" value={active} sub="can log in" icon={UserCheck} color="green" />
                    <StatCard label="Inactive Users" value={inactive} sub="access suspended" icon={UserX} color="amber" />
                    <StatCard label="Your Role" value="ADMIN" sub="full access" icon={ShieldCheck} color="purple" />
                </div>
                ...
                <div className="card overflow-hidden">
                    ...
                    {isLoading ? (
                        ...
                        <div className="p-8"><ErrorMessage message="Could not load users." /></div>
                    ) : (
                                ...
                                <tbody className="divide-y divide-slate-800/60">
                                    {(data ?? []).map(u => (
                                        <tr key={u.id} className="hover:bg-slate-800/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold ${u.role === 'ADMIN'
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg text-xs font-semibold ${u.isActive
                                            ...
                                            <td className="px-6 py-4">
                                                ...
                                                {u.id !== user?.id ? (
                                                    <button
                                                        ...
                                                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${u.isActive
                ...
                <div className="card p-6">
                    ...
                    <div className="mt-3 flex items-center gap-3">
                        <button
                            ...
                            disabled={sendNotif.isPending || !notifForm.userId || !notifForm.title || !notifForm.message}
                            className="btn-primary"
                        >
                            <Send className="w-3.5 h-3.5" />
                            {sendNotif.isPending ? 'Sending…' : 'Send'}
                        </button>
                        {sendNotif.isSuccess && <span className="text-xs text-green-400">Notification sent!</span>}
                        {sendNotif.isError && <span className="text-xs text-red-400">Failed to send.</span>}
                    </div>
                </div>
