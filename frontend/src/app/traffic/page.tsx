...
const LEVEL_FILTERS: Array<TrafficLevel | 'ALL'> = [
  'ALL',
  'LOW',
  'MEDIUM',
  'HIGH',
...
export default function TrafficPage() {
  ...
  return (
    <DashboardLayout>
      <Header
        title="Traffic Zones"
        subtitle={`${data?.length ?? 0} zones monitored`}
      />
      ...
      <div className="px-8 py-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {(['LOW', 'MEDIUM', 'HIGH'] as TrafficLevel[]).map((level) => (
            <div key={level} className="card p-4 flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${level === 'HIGH' ? 'bg-red-500' : level === 'MEDIUM' ? 'bg-amber-500' : 'bg-green-500'}`}
              />
              <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="lg:col-span-2 flex items-center gap-3">
                ...
                <input
                  ...
                  value={form.radius}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, radius: parseInt(e.target.value) }))
                  }
        ...
        {isLoading ? (
          ...
          <ErrorMessage message="Could not load traffic zones." />
        ) : filtered.length === 0 ? (
          <EmptyState
            icon={MapPin}
            title="No traffic zones"
            description="No zones match your filter."
          />
        ) : (
            ...
            {filtered.map((z) => (
              <TrafficZoneCard
                ...
                onUpdateDensity={
                  isAdmin
                    ? (density) =>
                        updateDensity.mutate({ zoneId: z.id, density })
                    : undefined
                }
              />
            ))}
