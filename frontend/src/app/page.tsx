...
export default function DashboardPage() {
  ...
  const zones = useQuery<TrafficZone[]>({
    queryKey: ['zones'],
  ...
  const incidentTrendData = Array.from({ length: 7 }, (_, i) => {
    ...
    const dayIncidents = (incidents.data ?? []).filter(
  ...
  const incidentTypeData = (
    ['ACCIDENT', 'CONSTRUCTION', 'ROAD_CLOSED', 'TRAFFIC_JAM'] as const
  ).map((t) => ({
    name: t.replace('_', ' '),
    incidents: (incidents.data ?? []).filter((i) => i.type === t).length,
    resolved: (incidents.data ?? []).filter(
      (i) => i.type === t && i.status === 'RESOLVED',
    ).length,
  }));
  ...
  const vehiclePieData = [
    ...
    {
      name: 'Inactive',
      value: vehicles.data?.filter((v) => v.status === 'INACTIVE').length ?? 0,
      color: '#64748b',
    },
    {
      name: 'Maintenance',
      value:
        vehicles.data?.filter((v) => v.status === 'MAINTENANCE').length ?? 0,
      color: '#f59e0b',
    },
  ].filter((d) => d.value > 0);
      ...
      <div className="px-8 py-6 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            icon={AlertTriangle}
            color="amber"
            trend={{ value: openIncidents > 0 ? -5 : 0, label: '' }}
          />
          <StatCard
            label="Traffic Zones"
            value={totalZones}
            sub={`${highZones} high density`}
            icon={MapPin}
            color="purple"
          />
          <StatCard
