    ...
    return (
        <div className={cn(
            ...
            <div className={cn(
                ...
                <Bell className={cn('w-4 h-4', notification.isRead ? 'text-slate-600' : 'text-brand-400')} />
            </div>

            <div className="flex-1 min-w-0">
                <p className={cn('text-sm', notification.isRead ? 'text-slate-400' : 'text-white font-medium')}>
                    {notification.message}
                </p>
                <div className="flex items-center gap-2 mt-1">
                    <Clock className="w-3 h-3 text-slate-600" />
                    <span className="text-xs text-slate-600">{relativeTime(notification.createdAt)}</span>
                    {!notification.isRead && (
