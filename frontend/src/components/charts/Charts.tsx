...
interface AreaData { name: string; value: number; }
    ...
    return (
        <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="value" stroke="#4f62ff" strokeWidth={2}
                    fill="url(#colorVal)" dot={false} activeDot={{ r: 4, fill: '#4f62ff' }} />
            </AreaChart>
        </ResponsiveContainer>
    );
}
...
interface BarData { name: string; incidents: number; resolved: number; }

export function IncidentBarChart({ data }: { data: BarData[] }) {
    return (
        <ResponsiveContainer width="100%" height={200}>
            <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }} barCategoryGap="30%">
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#64748b' }} axisLine={false} tickLine={false} />
